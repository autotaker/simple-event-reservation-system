...
test.describe('US-09 予約状況をCSVでエクスポートできる', () => {
  test('未認証でCSVエクスポートAPIにアクセスすると401になる', async ({ request }) => {
    const reservationsResponse = await request.get(
      'http://127.0.0.1:8080/api/admin/exports/reservations',
    );
    expect(reservationsResponse.status()).toBe(401);

    const checkInsResponse = await request.get(
      'http://127.0.0.1:8080/api/admin/exports/session-checkins',
    );
  ...
  test('運営は予約一覧とチェックインCSVをUTF-8で出力できる', async ({ page, request }) => {
    ...
    expect(checkInsContent).toContain(
