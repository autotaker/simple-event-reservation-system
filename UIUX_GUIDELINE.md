...
- 主タスク（閲覧/判断/予約/登録など）が、主要デバイス（特にモバイル）で完了できない、または完了までに著しい混乱を伴う
- 操作結果が分からず、ユーザーが次行動を判断できない（成功/失敗/進行中の状態フィードバック欠落・不整合）
- 誤操作を誘発しやすく、かつ回復導線がない（取り消し不能、修正手段不明、戻り導線不足）
- 制約エラー時の説明不足により、ユーザーが原因と解決策を理解できず離脱リスクが高い
- モック中心の構成から本番実装構成へ移行できていない（運用ルール違反）
- 単一ViewにUI責務が集中し、合意済みの責務分割が実装に反映されていない（運用ルール違反）
- 実コンポーネント基準のStorybook/検証導線が不足し、UX回帰を安定検知できない（運用ルール違反）

注記:
- 上記3項目（実装構造/Storybook運用）は、主タスク影響の有無にかかわらず `P1` として扱う。
...
- 主タスクは完了できるが、判断や操作に継続的な負荷があり、完了効率が明確に低下する
- 誤解を招く文言や状態表示により、誤操作・再操作が発生しやすい
- モバイルで主導線に追加操作（スクロール/切替/展開など）が必要で、補助設計不足により認知負荷が高い
- エラーや制約の説明が不十分で、ユーザーが試行錯誤しないと解決できない

運用目安:
- 次スプリントまたは直近リリース前に対応方針を確定する。

#### P3（中優先度: 改善提案）

以下に該当する場合は `P3` とする。

- 主タスク完了や理解には影響しないが、体験品質（可読性・一貫性・安心感）を下げる
- 文言トーン、余白、強調、フィードバックタイミングなどの微調整で改善可能
- 特定条件でのみ発生し、回避が容易な軽微な使いづらさ

運用目安:
- バックログに登録し、関連改修タイミングで計画的に改善する。

### 7.3 モバイルUX判定（必須）

モバイル端末では、個別UI要素ではなく「主タスクを迷わず完了できるか」を基準に判定する。

1. 対象画面を **390x844** で確認する（必要に応じて 360x800 も併記）。
2. 主タスク（閲覧 -> 判断 -> 操作完了）が、以下の観点で成立するかを確認する。
   - 情報認知: 重要情報（タイトル/状態/操作対象）が一目で把握できる
   - 操作負荷: タップ移動や視線移動が過剰でない
   - フィードバック: 操作結果（成功/失敗/状態変化）が即時かつ明確に伝わる
   - 回復性: 誤操作や制約違反時に、次の行動が分かる導線がある
3. レイアウト制約により追加操作（スクロール/切替/展開など）が必要な場合は、以下の有無を確認する。
   - 現在位置と対象を見失わない補助設計
   - 次操作を示す明示的なガイダンス
   - 同等目的を達成できる代替導線
4. 上記が不足し、主タスクの完了効率または理解容易性が落ちる場合は `P2` 以上で指摘する。
...
- モバイルUXに関する指摘では、検証した画面サイズ（例: `390x844`）と、主タスクのどこで負荷が発生したかを必ず明記する。
...
- モバイルUXを理由に指摘する場合は、以下を必ず明記する。
  - 対象ビューポート（例: `390x844`）
  - 主タスク上の阻害ポイント（どのステップで迷い/負荷が出るか）
  - 改善案の優先順位（例: 情報構造 -> 操作導線 -> 文言/フィードバック）
