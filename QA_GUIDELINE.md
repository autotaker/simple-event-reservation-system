...
- UIリファクタ・レイアウト移行時は、既存E2Eの期待値を同一QA作業内で更新し、旧UI前提の文言期待を残さない。
- ロケータは曖昧一致を避け、`getByRole` の厳密名（例: `/^Session 1$/`）または `frontend/e2e/support/ui.ts` の専用ヘルパーを優先する。
- 検証観点は文言連結（例: `17:15 | Track A`）ではなく、見出し・行ヘッダ・列ヘッダ・状態ラベルなどの意味単位で分解してアサートする。
- 現在のカバレッジ:
  - `US-01` ゲストログイン
  - `US-02` キーノート予約主要導線
- ローカル実行:
